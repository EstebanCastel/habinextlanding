...
export default function Experiencia({ inicial, fase, sitio, li }: { inicial: Vista | null; fase: Fase; sitio: string; li?: string }) {
  ...
  const [abierta, setAbierta] = useState<MisionId | null>(() =>
    li === "ok"
      ? (MISIONES.find((m) => m.red === "linkedin" && !inicial?.misiones[m.id] && !(m.fase === "evento" && fase === "antes"))?.id ?? null)
      : null
  );

  const irAMision = useCallback((m: MisionId) => {
    setAbierta(m);
  ...
  return (
    <>
      <div className="mx-auto max-w-[1400px] px-5 pb-20 sm:px-8 md:px-14 md:pb-28 lg:px-20">
        ...
        <Carnet yo={yo} alCambiar={setYo} alPublicar={irAMision} />
      </div>
      ...
      <div className="s-ink border-t border-white/8">
        <div className="mx-auto max-w-[1400px] px-5 py-20 sm:px-8 md:px-14 md:py-28 lg:px-20">
          ...
          <Misiones yo={yo} fase={fase} sitio={sitio} li={li} alCambiar={setYo} asegurar={asegurar} abierta={abierta} alAbrir={setAbierta} />
        </div>
      </div>
    </>
  );
